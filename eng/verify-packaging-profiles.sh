...
grep -Fq 'assert_windows_json_inventory_comparison' "$notice_verifier"
grep -Fq 'Microsoft.Extensions.DependencyInjection.Abstractions' "$notice_verifier"
! grep -Fq 'mapfile' "$notice_verifier"
! grep -Fq '@tsv' "$notice_verifier"
