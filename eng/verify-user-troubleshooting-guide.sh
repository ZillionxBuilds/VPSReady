...
grep -Fq 'surface does not render an error code. Selecting an entry shows a fixed' "$guide"
grep -Fq 'local key/config failures refer to local state' "$guide"
grep -Fq 'remote failures retain a remote-state warning' "$guide"
