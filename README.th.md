...
แม้แพ็กเกจจะรวม .NET runtime แล้ว แต่ Linux ยังต้องใช้ไลบรารีของเดสก์ท็อป
หากเปิดแอปบน Ubuntu แบบติดตั้งขั้นต่ำแล้วพบว่าไม่มี `libfontconfig.so.1`
ให้ติดตั้ง `libfontconfig1` **บนเครื่องเดสก์ท็อปที่รัน VPSReady** ไม่ใช่บน VPS
ที่แอปจัดการ ส่วนการรันชุดทดสอบ OpenSSH config บน Linux แบบขั้นต่ำต้องมี
`openssh-client` ซึ่งเป็นเครื่องมือสำหรับทดสอบ ไม่ใช่ dependency ของการเชื่อมต่อ
SSH.NET ในตัวแอป
